...
[HBase一篇较好的入门介绍](https://www.guru99.com/hbase-tutorials.html)

HBase的bulk loading值得一看：[How-to: Use HBase Bulk Loading, and Why](https://blog.cloudera.com/blog/2013/09/how-to-use-hbase-bulk-loading-and-why/)

HBase性能调优：[HBase Performance Tuning | Ways For HBase Optimization](https://data-flair.training/blogs/hbase-performance-tuning/)

[HBase coprocessor介绍](https://blogs.apache.org/hbase/entry/coprocessor_introduction)



HBase性能测试命令示例：`bin/hbase pe --table="testtable" --rows=100000 sequentialWrite 10`，如果遇到FileNotFoundException，试试`HADOOP_CLASSPATH=$(${HBASE_HOME}/bin/hbase classpath) ${HADOOP_HOME}/bin/hadoop jar ${HBASE_HOME}/lib/hbase-it-1.2.9-tests.jar org.apache.hadoop.hbase.PerformanceEvaluation --table="testtable" --rows=100000 sequentialWrite 10`，具体解释见下面。

#### 提交HBase MapReduce作业时的jar依赖问题

如果直接运行这个命令`bin/hbase org.apache.hadoop.hbase.mapreduce.RowCounter gubapost`（统计某一个hbase表的行数），很可能遇到如下问题：

2018-12-10 17:36:28,765 INFO  [main] mapreduce.JobSubmitter: Cleaning up the staging area file:/tmp/hadoop-yaming.ma/mapred/staging/hdfs1792523924/.staging/job_local1792523924_0001
Exception in thread "main" java.io.FileNotFoundException: File does not exist: hdfs://hadoop-archive-cluster/datayes/bigdata/hbase-1.2.9/lib/hbase-prefix-tree-1.2.9.jar
...
`HADOOP_CLASSPATH=$(${HBASE_HOME}/bin/hbase classpath) ${HADOOP_HOME}/bin/hadoop jar ${HBASE_HOME}/lib/hbase-server-1.2.9.jar rowcounter gubapost`，具体解释可以见[HBase, MapReduce, and the CLASSPATH](https://archive.cloudera.com/cdh5/cdh/5/hbase-0.98.6-cdh5.3.0/book/mapreduce.html#hbase.mapreduce.classpath)

#### 提交HBase MapReduce作业时的LocalJobRunner问题

不管是前面的RowCounter还是PerformanceEvaluation，都碰到过其实没有提交Map Reduce作业，而是在跑本地作业的情况，因为当时没有提供正确的mapred-site.xml文件，其中的`mapreduce.framework.name`设置默认是local，而不是yarn，导致其实都提交了本地作业。

# ElasticSearch介绍



[ElasticSearch官方安装与配置文档（Debian版）](https://www.elastic.co/guide/en/elasticsearch/reference/current/deb.html)

[Kibana官方安装与配置文档（Debian版）](https://www.elastic.co/guide/en/kibana/current/deb.html)

ElasticSearch API极简入门：[A Practical Introduction to Elasticsearch](https://www.elastic.co/blog/a-practical-introduction-to-elasticsearch)